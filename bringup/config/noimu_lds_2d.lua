...
TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.linear_search_window = 0.05  -- 並進探索範囲を狭める（0.15→0.1）
TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.angular_search_window = math.rad(45.)  -- 角度ずれ対策 探索範囲を広げる
TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.translation_delta_cost_weight = 0.8  -- 並進コスト重み
TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.rotation_delta_cost_weight = 2.0  -- 回転コスト重み
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.occupied_space_weight = 30.0  -- 占有空間の重みを上げる（20→30）
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.translation_weight = 20.0  -- 並進の重みを上げる（10→15）
...
TRAJECTORY_BUILDER_2D.motion_filter.max_angle_radians = math.rad(2.0)  -- 角度閾値を小さくする（10°→5°→角度ずれ対策3°）
