...
options = {
  ...
  published_frame = "base_footprint",  -- オドメトリのフレーム名
  odom_frame = "odom",  -- オドメトリのフレーム名
  provide_odom_frame = true,  -- オドメトリフレームをCartographerが提供
  ...
  imu_sampling_ratio = 0.0,  -- IMUなしの設定
...
-- 相関スキャンマッチングの調整（試行錯誤ポイント①）
TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.linear_search_window = 0.1  -- 並進探索範囲を狭める（0.15→0.1）
TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.angular_search_window = math.rad(45.)  -- 角度ずれ対策として探索範囲を広げる
...
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.rotation_weight = 60.0  -- 角度ずれ対策として40から60に増加

-- 小型ロボット向けにモーションフィルタを調整（試行錯誤ポイント③）
...
TRAJECTORY_BUILDER_2D.motion_filter.max_angle_radians = math.rad(3.0)  -- 角度閾値を小さくする（10°→5°→3°）

-- サブマップサイズを調整（試行錯誤ポイント④）
...
-- POSE_GRAPH: ループクロージャーと軌跡最適化に関する設定（試行錯誤ポイント⑥）
POSE_GRAPH.optimization_problem.huber_scale = 1e2  -- 外れ値に対する堅牢性の調整
POSE_GRAPH.optimize_every_n_nodes = 30  -- より頻繁に最適化
POSE_GRAPH.constraint_builder.min_score = 0.7  -- 角度ずれ対策として0.65→0.7に拘束条件とみなす最小スコア
POSE_GRAPH.constraint_builder.global_localization_min_score = 0.8  -- 角度ずれ対策として0.7→0.8に
POSE_GRAPH.global_sampling_ratio = 0.003  -- グローバルサンプリング比率
POSE_GRAPH.global_constraint_search_after_n_seconds = 10.

-- Create 2は小型なのでループ検出を調整（試行錯誤ポイント⑦）
