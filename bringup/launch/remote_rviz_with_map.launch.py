...
def generate_launch_description():
    ...
    map_file = LaunchConfiguration('map_file', default='/home/mkouda/map.yaml')
    ...
    declare_map_file_cmd = DeclareLaunchArgument(
        'map_file',
        default_value='/home/mkouda/map.yaml',
        description='Full path to map yaml file to load'
    )
    
    # ローカルマップサーバー（ネットワーク負荷軽減用）
    map_server_node = Node(
        package='nav2_map_server',
        executable='map_server',
        name='local_map_server',  # 名前を変更してロボット側との競合を避ける
        output='screen',
        parameters=[{
            'use_sim_time': use_sim_time,
            'yaml_filename': map_file,
            'topic_name': 'local_map',  # トピック名を変更してロボット側と区別
            'frame_id': 'map'
        }]
    )
    
    # RViz設定ファイルのパス
    rviz_config_path = os.path.join(bringup_dir, 'rviz', 'config.rviz')
...
#     # Rviz用のカスタム設定ファイルを作成
#     os.makedirs(os.path.join(bringup_dir, 'rviz'), exist_ok=True)
#     rviz_config_path = os.path.join(bringup_dir, 'rviz', 'remote_view.rviz')
...
#         Reliability Policy: Reliable
#         Value: /goal_pose
#     - Class: rviz_plugins/PublishPoint
#       Single click: true
#       Topic:
#         Depth: 5
#         Durability Policy: Volatile
